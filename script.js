...
const account2 = {
  ...
  movementsDates: [
    ...
    '2026-01-11T06:04:23.907Z',
    '2026-01-11T14:18:46.235Z',
    '2026-01-13T16:33:06.386Z',
...
const formatMovementDate = function (date, acc) {
  const calcDaysPassed = (date1, date2 = new Date()) =>
    Math.abs(date2 - date1) / (1000 * 60 * 60 * 24);
  const daysPassed = Math.floor(calcDaysPassed(date));
...
const calcDisplaySummary = function (acc) {
  ...
  const formattedIncomes = formatNumbers(incomes, acc.locale, acc.currency);
  labelSumIn.textContent = formattedIncomes;

  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  const formattedOut = formatNumbers(out, acc.locale, acc.currency);
  ...
  const formattedinterest = formatNumbers(interest, acc.locale, acc.currency);
